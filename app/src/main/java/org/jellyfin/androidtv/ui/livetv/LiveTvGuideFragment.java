...
public class LiveTvGuideFragment extends Fragment implements LiveTvGuide,View.OnKeyListener {
    ...
    private boolean onKeyUp(int keyCode,KeyEvent event) {
        switch(keyCode) {
            ...
            case KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE:
                if((mDetailPopup==null||!mDetailPopup.isShowing())&&mSelectedProgram!=null&&mSelectedProgram.getChannelId()!=null) {
                    playbackHelper.getValue().retrieveAndPlay(mSelectedProgram.getChannelId(),false,requireContext());
                    return true;
                }
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                if(requireActivity().getCurrentFocus() instanceof ProgramGridCell&&mSelectedProgramView!=null&&((ProgramGridCell)mSelectedProgramView).isLast()) {
                    requestGuidePage(mCurrentGuideEnd);
                }
                break;
            case KeyEvent.KEYCODE_DPAD_LEFT:
                if(requireActivity().getCurrentFocus() instanceof ProgramGridCell&&mSelectedProgramView!=null&&((ProgramGridCell)mSelectedProgramView).isFirst()&&mSelectedProgram.getStartDate().isAfter(LocalDateTime.now())) {
                    focusAtEnd=true;
    ...
    private void showDatePicker() {
        ...
        dateDialog=new AlertDialog.Builder(requireContext()).setTitle(R.string.lbl_select_date).setView(scrollPane).setNegativeButton(R.string.btn_cancel,new DialogInterface.OnClickListener() {
            @Override
    ...
    private void requestGuidePage(final LocalDateTime startTime) {
        new AlertDialog.Builder(requireContext()).setTitle(R.string.lbl_load_guide_data).setMessage(startTime.isAfter(mCurrentGuideStart)?getString(R.string.msg_live_tv_next,getGuideHours()):getString(R.string.msg_live_tv_prev,getGuideHours())).setPositiveButton(R.string.lbl_yes,new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog,int which) {
                pageGuideTo(startTime);
            }
        }).setNegativeButton(R.string.lbl_no,null).show();
    }
    ...
    private LinearLayout getProgramRow(List<BaseItemDto> programs,UUID channelId) {
        ...
        if(programs.size()==0) {
            ...
            int slot=0;
            do {
                BaseItemDto empty=LiveTvGuideFragmentHelperKt.createNoProgramDataBaseItem(getContext(),channelId,mCurrentGuideStart.plusMinutes(30l*slot),mCurrentGuideEnd.plusMinutes(30l*(slot+1)));
        ...
        for(BaseItemDto item:programs) {
            ...
            if(start.isAfter(prevEnd)) {
                BaseItemDto empty=LiveTvGuideFragmentHelperKt.createNoProgramDataBaseItem(getContext(),channelId,prevEnd,start);
                ProgramGridCell cell=new ProgramGridCell(requireContext(),this,empty,false);
                cell.setId(currentCellId++);
        ...
        if(prevEnd.isBefore(mCurrentGuideEnd)) {
            BaseItemDto empty=LiveTvGuideFragmentHelperKt.createNoProgramDataBaseItem(getContext(),channelId,prevEnd,mCurrentGuideEnd);
            ProgramGridCell cell=new ProgramGridCell(requireContext(),this,empty,false);
            cell.setId(currentCellId++);
    ...
    private void fillTimeLine(LocalDateTime start,int hours) {
        ...
        mCurrentGuideStart=mCurrentGuideStart.withMinute(mCurrentGuideStart.getMinute()>=30?30:0).withSecond(0).withNano(0);
